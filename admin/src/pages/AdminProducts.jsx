        ...
        <div className="adm-wrap" style={{ minHeight: "100vh", background: "#f0f4ff", padding: "24px 16px 60px" }}>
            ...
            <div style={{ maxWidth: 1080, margin: "0 auto" }}>
                ...
                <div className="uk-stats-grid">
                    {[
                        { label: "Total", value: products.length, color: "#2563eb", bg: "#eff6ff", icon: "📦", border: "#bfdbfe" },
                        { label: "In Stock", value: products.filter(p => p.inStock).length, color: "#059669", bg: "#f0fdf4", icon: "✅", border: "#bbf7d0" },
                        { label: "Out of Stock", value: products.filter(p => !p.inStock).length, color: "#ef4444", bg: "#fef2f2", icon: "❌", border: "#fecaca" },
                        {
                            label: "Avg Price",
                            value: `₹${products.length
                                ? Math.round(products.reduce((s, p) => s + p.price, 0) / products.length).toLocaleString("en-IN")
                                : 0}`,
                            color: "#7c3aed", bg: "#f5f3ff", icon: "💰", border: "#ddd6fe",
                        },
                    ].map(({ label, value, color, bg, icon, border }) => (
                        <div key={label} className="uk-stat-card" style={{
                            background: "#fff",
                            ...
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                                <p style={{ fontSize: 10, color: "#94a3b8", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", margin: 0 }}>
                                    {label}
                                </p>
                                <div style={{
