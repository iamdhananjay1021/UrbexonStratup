...
export const useCheckout = (buyNowItem = null) => {
    ...
    /* ── COD availability ── */
    const [codStatus, setCodStatus] = useState(null); // null | "available" | "coming_soon"
    ...
    useEffect(() => {
        ...
        const checkCOD = async () => {
            try {
                ...
                setCodStatus(data.codStatus || (data.codAllowed ? "available" : "coming_soon"));
                setCodDistance(data.distanceKm);
                setDeliveryETA(data.deliveryETA || "");
            } catch {
                setCodStatus("coming_soon");
            } finally {
