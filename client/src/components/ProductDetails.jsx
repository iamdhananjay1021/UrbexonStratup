...
const ShareModal = ({ product, onClose }) => {
    ...
    return (
        <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 99999, background: "rgba(0,0,0,0.55)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: "8vh", padding: "8vh 16px 0" }}>
            <div onClick={e => e.stopPropagation()} style={{ background: "#fff", width: "100%", maxWidth: 380, borderRadius: 16, boxShadow: "0 20px 60px rgba(0,0,0,.25)", maxHeight: "82vh", overflowY: "auto", animation: "mcSlideDown .22s ease" }}>
                ...
                <div style={{ margin: "12px 20px", background: "#fafafa", borderRadius: 10, padding: "10px 12px", display: "flex", alignItems: "center", gap: 12, border: "1px solid #f0f0f0" }}>
                    <div style={{ width: 48, height: 48, borderRadius: 8, background: "#fff", border: "1px solid #eee", overflow: "hidden", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        {product.images?.[0]?.url ? <img src={imgUrl.detail(product.images[0].url)} alt={product.name} style={{ width: "100%", height: "100%", objectFit: "contain", padding: 4 }} /> : <span style={{ fontSize: 20 }}>🎁</span>}
                    </div>
                ...
                <div style={{ margin: "0 20px 20px", display: "flex", alignItems: "center", gap: 8, background: "#f5f5f5", borderRadius: 8, padding: "8px 12px", border: "1px solid #e0e0e0" }}>
                    ...
                    <button onClick={async () => { try { await navigator.clipboard.writeText(url); setCopied(true); setTimeout(() => setCopied(false), 2000); } catch { } }}
...
const ZoomModal = ({ src, alt, onClose }) => {
    ...
    return (
        <div onClick={onClose} style={{
            ...
            <button onClick={onClose} style={{ position: "fixed", top: 16, right: 16, width: 40, height: 40, background: "rgba(255,255,255,.15)", border: "1px solid rgba(255,255,255,.3)", borderRadius: 4, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><FaTimes size={16} color="#fff" /></button>
...
const ProductDetails = () => {
    ...
    const highlightEntries = useMemo(() => {
        ...
        return product.highlights instanceof Map ? [...product.highlights.entries()] : Object.entries(product.highlights);
    }, [product?.highlights]);
        ...
        (async () => {
            try {
                ...
                const [{ data: prod }, { data: related }] = await Promise.all([
                    ...
                    api.get(`/products/${id}/related`, { signal: ctrl.signal }),
                ]);
    ...
    const handleNotifyMe = useCallback(async () => {
        if (!notifyEmail.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(notifyEmail)) { setNotifyError("Enter a valid email"); return; }
        try {
            setNotifySubmitting(true); setNotifyError("");
            const key = `notify_${product._id}`;
            const arr = JSON.parse(localStorage.getItem(key) || "[]");
            if (!arr.includes(notifyEmail.trim())) { arr.push(notifyEmail.trim()); localStorage.setItem(key, JSON.stringify(arr)); }
    ...
    const handleSubmitReview = useCallback(async (e) => {
        ...
        try {
            ...
            const { data } = await api.get(`/products/${id}`); setProduct(data);
    ...
    const handleDeleteReview = useCallback(async (rid) => {
        try { await api.delete(`/reviews/${rid}`); await fetchReviews(product._id); setMyRating(0); setMyComment(""); } catch { }
    ...
    return (
        <>
            ...
            <style>{`
                ...
                .pd-size:hover  { border-color:#2874f0; color:#2874f0; }
                .pd-size.active { border:2px solid #212121; background:#fff; color:#212121; font-weight:700; }
                .pd-size.oos    { color:#ccc; cursor:not-allowed; background:#fafafa; }
                        ...
                        <div className="pd-img-wrap" style={{ borderRight: "1px solid #f0f0f0", padding: "24px 20px" }}>
                            ...
                            <div style={{ position: "relative" }}>
                                ...
                                <div style={{ position: "absolute", top: 10, left: 10, zIndex: 2, display: "flex", flexDirection: "column", gap: 4 }}>
                                    ...
                                    {!product.inStock && <span style={{ background: "#212121", color: "#fff", fontSize: 10, fontWeight: 800, padding: "3px 8px", borderRadius: 2 }}>SOLD OUT</span>}
                                ...
                                {/* Hero image — tall like Monte Carlo */}
                                <div
                        ...
                        <div className="pd-info-wrap" style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: 0 }}>
                            ...
                            <div style={{ marginBottom: 4 }}>
                                ...
                                {hasDiscount && savedAmount > 0 && (
                                    <p style={{ fontSize: 13, color: "#388e3c", fontWeight: 600, marginTop: 4 }}>You save ₹{savedAmount.toLocaleString("en-IN")}</p>
                                )}
                            </div>
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                                {product.inStock ? (<>
                                    ...
                                    {product.stock > 0 && product.stock <= 10 && (
                                        <span style={{ fontSize: 12, color: "#ff6161", fontWeight: 600, background: "#fff3f3", padding: "2px 8px", borderRadius: 3 }}>Only {product.stock} left</span>
                                    )}
                                </>) : (<>
                            ...
                            {product.sizes?.length > 0 && (
                                <div style={{ marginBottom: 18 }}>
                                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                                        ...
                                        <button style={{ fontSize: 12, color: "#2874f0", background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>Size Guide</button>
                                    </div>
                                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                                        {product.sizes.map(sz => (
                                            <button key={sz} onClick={() => setSelectedSize(sz)} className={`pd-size ${selectedSize === sz ? "active" : ""}`}>{sz}</button>
                                        ))}
                                    </div>
                                    {!selectedSize && <p style={{ fontSize: 12, color: "#ff6161", marginTop: 8, fontWeight: 600 }}>↑ Please select a size to continue</p>}
                                </div>
                            )}
                            ...
                            {product.isCustomizable && (
                                <div style={{ border: "1px solid #ffe0b2", background: "#fffde7", borderRadius: 4, padding: 16, marginBottom: 18 }}>
                                    ...
                                    <div style={{ marginBottom: 10 }}>
                                        <label style={{ fontSize: 11, fontWeight: 700, color: "#bf360c", display: "flex", alignItems: "center", gap: 4, marginBottom: 6 }}><FaPencilAlt size={9} /> Name / Message</label>
                                        <input type="text" value={customText} onChange={e => setCustomText(e.target.value)} placeholder="e.g. Happy Birthday Rahul! 🎂"
                                            style={{ width: "100%", padding: "8px 12px", border: "1px solid #ffe0b2", borderRadius: 4, fontSize: 13, outline: "none", background: "#fff" }} />
                                    </div>
                                    <div style={{ marginBottom: 10 }}>
                                        <label style={{ fontSize: 11, fontWeight: 700, color: "#bf360c", display: "flex", alignItems: "center", gap: 4, marginBottom: 6 }}><FaUpload size={9} /> Upload Design</label>
                                        {!customImagePreview ? (
                                            ...
                                            <div style={{ position: "relative", display: "inline-block" }}>
                                                ...
                                                <button onClick={removeCustomImage} style={{ position: "absolute", top: -8, right: -8, width: 20, height: 20, background: "#f44336", color: "#fff", border: "none", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}><FaTimes size={9} /></button>
                                    ...
                                    <div>
                                        <label style={{ fontSize: 11, fontWeight: 700, color: "#bf360c", display: "flex", alignItems: "center", gap: 4, marginBottom: 6 }}><FaStickyNote size={9} /> Special Instructions</label>
                                        <textarea value={customNote} onChange={e => setCustomNote(e.target.value)} placeholder="e.g. White background, bold font..." rows={2}
                                            style={{ width: "100%", padding: "8px 12px", border: "1px solid #ffe0b2", borderRadius: 4, fontSize: 13, outline: "none", resize: "none", background: "#fff" }} />
                                    </div>
                                </div>
                            )}
                            ...
                            {product.inStock ? (
                                <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                                    <button onClick={handleAddToCart} className={`pd-btn pd-cart ${inCart ? "in-cart" : ""}`}
                                ...
                                <div style={{ marginBottom: 16 }}>
                                    <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
                                        <button disabled className="pd-btn" style={{ background: "#f0f0f0", color: "#bdbdbd", cursor: "not-allowed", flex: 1 }}><FaShoppingCart size={16} /> Add to Cart</button>
                                        <button disabled className="pd-btn" style={{ background: "#f0f0f0", color: "#bdbdbd", cursor: "not-allowed", flex: 1 }}><FaBolt size={16} /> Buy Now</button>
                                    </div>
                                    {notifySuccess ? (
                                        ...
                                        <div style={{ border: "1px solid #e0e0e0", borderRadius: 4, padding: 14 }}>
                                            ...
                                            {showNotifyInput ? (
                                                <div style={{ display: "flex", gap: 8 }}>
                                                    <input type="email" value={notifyEmail} onChange={e => { setNotifyEmail(e.target.value); setNotifyError(""); }} placeholder="your@email.com"
                    ...
                    <div style={{ borderTop: "1px solid #f0f0f0" }}>
                        <div style={{ display: "flex", borderBottom: "1px solid #f0f0f0", overflowX: "auto", paddingLeft: 16 }}>
                            {[
                                ...
                                <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`pd-tab ${activeTab === tab.key ? "active" : ""}`}>{tab.label}</button>
                            ))}
                        </div>
                                    ...
                                    {user ? (
                                        <form onSubmit={handleSubmitReview} style={{ marginBottom: 28, paddingBottom: 24, borderBottom: "1px solid #f0f0f0" }}>
                                            ...
                                            <textarea value={myComment} onChange={e => setMyComment(e.target.value)} rows={3} placeholder="Share your experience…"
                                                style={{ width: "100%", border: "1px solid #e0e0e0", borderRadius: 4, padding: "10px 12px", fontSize: 13, marginBottom: 10, outline: "none", resize: "none", fontFamily: "'Roboto',sans-serif" }} />
                                            {reviewError && <p style={{ color: "#f44336", fontSize: 12, marginBottom: 8 }}>{reviewError}</p>}
                                            {reviewSuccess && <p style={{ color: "#388e3c", fontSize: 12, marginBottom: 8, display: "flex", alignItems: "center", gap: 4 }}><FaCheckCircle size={10} /> Review submitted!</p>}
                                            <button type="submit" disabled={submitting}
                                                style={{ padding: "10px 24px", background: "#2874f0", color: "#fff", border: "none", borderRadius: 4, fontWeight: 700, fontSize: 13, cursor: "pointer", opacity: submitting ? .6 : 1 }}>
                                                {submitting ? "Submitting…" : "Submit Review"}
                                            </button>
                                        </form>
                                    ) : (
                                        <button onClick={() => navigate("/login")} style={{ marginBottom: 24, padding: "10px 24px", border: "1px solid #2874f0", color: "#2874f0", background: "#fff", borderRadius: 4, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                                                    ...
                                                    <div key={r._id} style={{ border: "1px solid #f0f0f0", borderRadius: 4, padding: 16 }}>
                                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                                                            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                                                                <div style={{ width: 34, height: 34, background: "#2874f0", color: "#fff", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, flexShrink: 0 }}>{r.name?.[0]?.toUpperCase() || "U"}</div>
                                                                <div>
                                                                    <p style={{ fontWeight: 600, fontSize: 13, color: "#212121" }}>{r.name}</p>
                                                                    <StarRow value={r.rating} size={11} />
                                                                </div>
                                                            </div>
                                                            {isOwn && <button onClick={() => handleDeleteReview(r._id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#ccc", padding: 4 }}><FaTrash size={12} /></button>}
                                                        </div>
