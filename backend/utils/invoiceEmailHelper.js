 ...
 * ─────────────────────────────────────────────────────────
 * Generates professional PDF invoice buffer using PDFKit
 * Used for email attachments + direct download
 * ─────────────────────────────────────────────────────────
 */
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const orderMeta = [
                ...
                ["Payment", isCOD ? "Cash on Delivery" : (isPaid ? "Online Paid ✅" : "Online")],
            ...
            /* ── AWB / TRACKING (if shipped) ── */
            if (order.shipping?.awbCode) {
                doc.font("Helvetica-Bold").fontSize(8).fillColor(C.mid).text("AWB:", M, y);
                doc.font("Helvetica").fontSize(8).fillColor(C.dark).text(order.shipping.awbCode, M + 40, y);
                if (order.shipping.courierName) {
                    doc.font("Helvetica").fontSize(8).fillColor(C.mid).text(`via ${order.shipping.courierName}`, M + 120, y);
                }
                y += 18;
            }
            ...
            (order.items || []).forEach((item, i) => {
                ...
                doc.font("Helvetica").fontSize(9).fillColor(C.mid)
                    .text(i + 1, COL.no, y + 10, { width: CW2.no, align: "center" });
                doc.font("Helvetica-Bold").fontSize(9).fillColor(C.dark)
                    .text(item.name || "Product", COL.desc, y + 10, { width: CW2.desc - 5, ellipsis: true });
            ...
            doc.font("Helvetica-Bold").fontSize(11).fillColor(C.white)
                .text("GRAND TOTAL", TX + 5, y + 7)
                .text(rs(order.totalAmount || subtotal), TX + TL, y + 7, { width: TV - 5, align: "right" });
            y += 40;
            ...
            doc.font("Helvetica").fontSize(8).fillColor(C.mid)
                .text("Scan this QR to verify your invoice on urbexon.in. Urbexon guarantees 100% authentic products.", M + 80, qrY + 28, { width: CW - 100 });
            doc.font("Helvetica-Bold").fontSize(8).fillColor(C.zinc)
                .text("Secure Transaction  ·  Fast Delivery  ·  Easy Returns", M + 80, qrY + 55);

            /* ── FOOTER ── */
            const FY = 780;
            hline(FY, C.border);
            doc.font("Helvetica-Bold").fontSize(10).fillColor(C.primary)
                .text(`${SHOP.name} — ${SHOP.tagline}`, M, FY + 14, { align: "center", width: CW });
            doc.font("Helvetica").fontSize(8).fillColor(C.mid)
                .text(`Support: WhatsApp ${SHOP.phone}  |  Email: ${SHOP.email}  |  ${SHOP.website}`, M, FY + 28, { align: "center", width: CW });
